...
chat_attempt() {
  local name="$1" prompt="$2" attempt="$3" status sha client out payload
  out="$E/$name-attempt-$attempt.json"
  client="order003-${GITHUB_RUN_ID:-local}-$name-$attempt"
  payload=$(jq -nc --arg message "$prompt" '{message:$message,history:[]}')
  status=$(curl -sS --max-time 150 -D "$R/$name-$attempt.headers" -o "$R/$name-$attempt.body" -w '%{http_code}' -X POST "$BASE_URL/api/chat" -H 'content-type: application/json; charset=utf-8' -H "x-wia-client: $client" --data "$payload") || status=000
...
cf_read() {
  ...
  status=$(curl -sS --max-time 90 -H "Authorization: Bearer $CF_API_TOKEN_CLEAN" -H 'accept: application/json' -o "$R/$out.raw.json" -w '%{http_code}' "$CF_API$suffix") || status=000
...
if jq -e '.status==200' "$E/health.meta.json" >/dev/null 2>&1 && jq -e '.ok==true and .worker=="wia" and (.workset|type=="string" and length>0) and (.marker|type=="string" and length>0)' "$E/health.json" >/dev/null 2>&1; then
  ...
  record_failure REMOTE_HEALTH "GET /api/health failed HTTP or body gate"
fi
...
ACCOUNT_HASH=""
CF_ACCOUNT_ID_CLEAN=$(printf '%s' "${CF_ACCOUNT_ID:-}" | tr -d '\r\n\t ')
CF_API_TOKEN_CLEAN=$(printf '%s' "${CF_API_TOKEN:-}" | tr -d '\r\n')
if [ -z "$CF_API_TOKEN_CLEAN" ] || [ -z "$CF_ACCOUNT_ID_CLEAN" ]; then
  record_failure CLOUDFLARE_AUTH "DEPLOY_NOW or CLOUDFLARE_ACCOUNT_ID secret unavailable"
elif ! [[ "$CF_ACCOUNT_ID_CLEAN" =~ ^[A-Fa-f0-9]{32}$ ]]; then
  record_failure CLOUDFLARE_ACCOUNT_ID "Cloudflare account identifier secret has invalid format"
else
...
PY
      ...
      record_failure CLOUDFLARE_IDENTITY "Active deployment and version could not be resolved as one 100 percent deployment"
    fi
  else
    record_failure CLOUDFLARE_READ "Authenticated deployment or version GET failed"
  fi
fi
unset CF_API_TOKEN CF_ACCOUNT_ID CF_API_TOKEN_CLEAN CF_ACCOUNT_ID_CLEAN 2>/dev/null || true
...

{
  ...
  printf 'CAPTURED=%q\n' "$CAPTURED"
  printf 'SOURCE_SHA=%q\n' "$SOURCE_SHA"
  printf 'SOURCE_TREE=%q\n' "$SOURCE_TREE"
  printf 'MAIN_SHA=%q\n' "$MAIN_SHA"
  printf 'WORKSET=%q\n' "$WORKSET"
  printf 'MARKER=%q\n' "$MARKER"
  printf 'MODEL=%q\n' "$MODEL"
  printf 'CHAT1_MODEL=%q\n' "$CHAT1_MODEL"
  printf 'CHAT2_MODEL=%q\n' "$CHAT2_MODEL"
  printf 'DEPLOYMENT_ID=%q\n' "$DEPLOYMENT_ID"
  printf 'VERSION_ID=%q\n' "$VERSION_ID"
  printf 'DEPLOYMENT_CREATED_AT=%q\n' "$DEPLOYMENT_CREATED_AT"
  printf 'SOURCE_MATCH=%q\n' "$SOURCE_MATCH"
  printf 'FAILURE_STEP=%q\n' "$failure_step"
  printf 'FAILURE_ERROR=%q\n' "$failure_error"
} > "$E/status.env"
